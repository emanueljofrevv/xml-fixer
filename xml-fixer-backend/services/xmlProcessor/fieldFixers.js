...
const PA_EXCEPTIONS = ['MPI', 'ACT13', 'PB22', 'DBA', 'FEIN'];
...
const noLabelFields = [
    fieldsMapping.label,
    fieldsMapping.formIdStamp,
    fieldsMapping.button,
    fieldsMapping.container,
    fieldsMapping.checkbox,
    fieldsMapping.image,
    fieldsMapping.repeatingRowControl,
    fieldsMapping.dataGrid,
    fieldsMapping.uploadButton,
];
...
const textInTagsFields = [
    fieldsMapping.checkbox,
    fieldsMapping.button,
    fieldsMapping.idStamp,
    fieldsMapping.uploadButton,
];
...
function isRequiredField(field, labelText) {
    ...
    const isRequired = hasTextInField
        ? field.Text[0].includes('*')
...
function checkDistanceToBorder(form, field) {
    ...
    if (distanceToRightBorder < 30) {
        addToReport(`#### ${fieldName}`, `The field is less than 30px from the right border.`);
    }
}
...
function checkTabOrder(form, field, pageIndex, fieldIndex) {
    ...
    if (tabOrder !== 0) {
        if (fix.tabOrder) {
            form.FormPages[0].FormPage[pageIndex].FieldList[0].BaseField[fieldIndex].TabOrder[0] =
                0;
...
function findFieldLabeByProximity(field, fields) {
    ...
    const onlyLabels = fields.filter((f) => f.$['xsi:type'] === fieldsMapping.label);
...
function hasDefaultName(fieldName, fieldType = 'Default') {
    ...
    if (isDefaultName) {
        addToReport(`#### ${fieldName}`, `The field name \`${fieldName}\` has a default value.`);
    }
...
function hasDefaultText(field) {
    ...
    if (isDefaultText) {
        addToReport(`#### ${fieldName}`, `The field text \`${defaultText}\` has a default value.`);
    }
...
function isLabelOverlaping(field, fields) {
    ...
    fields.forEach((f) => {
        ...
        const isLabel = f.$['xsi:type'] === fieldsMapping.label;
        const isFormStamp = f.$['xsi:type'] === fieldsMapping.formIdStamp;
...
function isStrTitleCase(fieldName) {
    ...
    words.forEach((word) => {
        ...
        if (word) {
            ...
            const isRestOfWordLowercase = word.slice(1) === word.slice(1).toLowerCase();
...
function hasSpellingError(fieldName) {
    ...
    words.forEach((word) => {
        ...
        if (!isValid && !isException) {
            ...
            const suggestionMsg = suggestedWords.length > 0 ? suggestedWords : 'No suggestions';
...
function container(form, pIndex, fieldIndex) {
    ...
    const hasMoreThan1field = fields.filter((f) => f.ContainerId[0] === fieldID).length > 1;
...
function datagrid(form, pIndex, fieldIndex) {
    ...
    if (!hasDefaultName(fieldName, fieldsMapping.dataGrid)) {
        if (!checkTitleCase(fieldName)) {
            form = fixTitleCase(form, pIndex, fieldIndex);
        }
        form = checkAccessibility(form, pIndex, fieldIndex);
...
function image(form, pIndex, fieldIndex) {
    ...
    if (!hasDefaultName(fieldName, fieldsMapping.image)) {
        if (!checkTitleCase(fieldName)) {
            form = fixTitleCase(form, pIndex, fieldIndex);
        }
        form = checkAccessibility(form, pIndex, fieldIndex);
...
function rowRepeatingControl(form, pIndex, fieldIndex) {
    ...
    if (!hasDefaultName(fieldName, fieldsMapping.repeatingRowControl)) {
        if (!checkTitleCase(fieldName)) {
            form = fixTitleCase(form, pIndex, fieldIndex);
        }
        if (!hasSpellingError(fieldName)) {
...
function uploadButton(form, pIndex, fieldIndex) {
    ...
    if (!hasDefaultName(fieldName, fieldsMapping.uploadButton)) {
        if (!checkTitleCase(fieldName)) {
            form = fixTitleCase(form, pIndex, fieldIndex);
        }
        form = checkAccessibility(form, pIndex, fieldIndex);
    ...
    if (!isSimpleUpload) {
        if (fix.simpleUpload) {
            ...
            addToReport(`#### ${fieldName}`, `The field [${fieldName}] was changed to [${false}]`);
        }
